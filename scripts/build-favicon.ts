...
// geometry AND the exact same drawing (rect and paths, stroked outline,
// round caps/joins) that src/components/ui/Logo.tsx renders, sourced from
// src/lib/brandMark.ts. The only two things that differ from the header
...
import { CONTENT_LINES, PAGE } from '../src/lib/brandMark.ts';

const BG = '#0f172a';
const FG = '#fff';

const lines = CONTENT_LINES.map((line) => `<path d="M${line.x} ${line.y}H${line.x + line.w}" />`).join('\n      ');
